...
为什么需要它：
- 服务是纯 API（H5 前端已移除），直接用 curl 看到的是一大坨 JSON，不直观。
- 本脚本把「设计引擎 / 完整对话」的真实产出（DeepSeek 实跑）结构化打印出来，
  并附带「质量判断点」，你对着看就能判断设计质量达标与否。

运行环境（项目真实依赖装在 Python 3.12）：
    C:/Users/Capri/AppData/Local/Programs/Python/Python312/python.exe examples/live_check.py

说明：
- 一运行即真实调用 DeepSeek（产生少量额度消耗），不是 mock。
- 配置来自项目根目录 .env，import config 时自动加载，无需手动 export。
...
# 脚本位于 examples/ 子目录，把项目根目录加入 sys.path 才能 import config/tools/agent
...
def _print_plan(title: str, plan: dict) -> None:
    ...
    """
    from tools import _match_style
