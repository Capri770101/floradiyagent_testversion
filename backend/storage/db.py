...
def _run_async(coro):
    ...
    存储层已全 async：此处把同步入口（init_db / 模块级装配）对 async 存储的调用
    安全地驱动起来。无运行中的事件循环（测试、脚本、模块导入）时直接用
    ``asyncio.run``；已有运行循环（如 FastAPI lifespan 内调用 init_db）时，
    另起线程跑独立事件循环，避免 ``asyncio.run cannot be called from a running loop``。

    结束前异步 dispose 当前临时循环创建的 asyncpg 引擎：否则残留连接在
    PG 端保持 idle in transaction 并持锁，后续会话的 DDL/查询会被阻塞挂起。
    """
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
        return ex.submit(lambda: asyncio.run(_wrapper())).result()
...
def _get_pg_engine():
    ...
    if _PG_ENGINE is None:
        ...
        # NullPool：不缓存连接（每线程 _pg_conn 自行缓存一个），避免 QueuePool 额度
        # 被同步 shim 的连接占满后 connect() 无限阻塞（测试/多线程场景挂起）。
        # connect_timeout：连接握手超时；statement_timeout：单条 SQL 执行超时。
        _PG_ENGINE = create_engine(_sync_pg_url(), poolclass=NullPool, pool_pre_ping=True, future=True,
...
class _PGConn:
    ...
    def execute(self, sql, params=None):
        ...
        try:
            ...
            # 只读查询执行后立即提交：psycopg2 的 SELECT 会开启事务，若不提交则连接
            # 停留 idle in transaction 并持锁，阻塞后续会话的 DDL/查询（测试挂起根因）。
            head = (sql or '').strip().upper()
            if head.startswith('SELECT') or head.startswith('PRAGMA') or head.startswith('SHOW'):
                self._conn.commit()
        except Exception:
            self._conn.rollback()
            raise
...
def init_db() -> None:
    ...
    if settings.database_url and _pg_dialect() == 'postgresql':
        ...
        # 注意必须用同步 _reset_engine：async _dispose_engine 会跨循环 await
        # dispose 其他（已关闭）loop 的引擎，导致永久挂起。
        _dba._reset_engine()
        try:
    ...
    try:
        ...
        logger.warning('users.phone 唯一索引创建失败（存量重复数据），降级为普通索引: %s', exc)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_users_phone ON users(phone) WHERE phone IS NOT NULL AND phone != ''")
    try:
        conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_card_token ON orders(card_token) WHERE card_token IS NOT NULL AND card_token != ''")
