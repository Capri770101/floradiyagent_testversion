...
async def create_report_endpoint(req: ReportCreateRequest, request: Request) -> dict[str, Any]:
    ...
    item = await report_store.create_report(uid, req.target_type, req.target_id.strip(), req.reason.strip(), req.content.strip())
    return {'report': item}

@router.get('/reports')
async def list_reports_endpoint(request: Request, status: str='', limit: int=50, offset: int=0) -> dict[str, Any]:
    """管理后台举报列表（新→旧；status 过滤 + 分页）。"""
    await _require_admin(request)
    return await report_store.list_reports(status.strip(), limit, offset)

@router.post('/reports/{report_id}/handle')
async def handle_report_endpoint(report_id: str, req: ReportHandleRequest, request: Request) -> dict[str, Any]:
    ...
    try:
        item = await report_store.handle_report(report_id, req.status, admin_uid)
