...
async def user_chat_list(request: Request) -> dict[str, Any]:
    ...
    chats = await chat_store.list_user_chats(uid)
    return {'chats': chats}
...
async def user_chat_with_shop(shop_id: str, request: Request) -> dict[str, Any]:
    ...
    if not uid:
        raise HTTPException(status_code=401, detail='缺少用户身份')
    shop_name = await chat_store.get_shop_name(shop_id)
    chat = await chat_store.get_or_create_chat(shop_id, uid)
    messages = await chat_store.list_messages(chat['id'], chat_store.SENDER_USER)
...
async def user_chat_messages(chat_id: str, request: Request) -> dict[str, Any]:
    ...
    messages = await chat_store.list_messages(chat_id, chat_store.SENDER_USER)
...
async def user_send_message(chat_id: str, req: ChatSendRequest, request: Request) -> dict[str, Any]:
    ...
    chat = await chat_store.get_chat(chat_id)
    _own_chat_or_403(chat, uid)
    message = await chat_store.send_message(chat_id, chat_store.SENDER_USER, req.content.strip())
