...
def _reject_c_end(uid: str) -> None:
    """C 端登录接口拒绝非普通用户：管理员走管理后台，商家走商家端（角色隔离）。"""
    role = security.get_user_role(uid)
    if role == 'admin':
        raise HTTPException(status_code=403, detail='管理员账号请使用管理后台登录')
    if role == 'merchant':
        raise HTTPException(status_code=403, detail='商家账号请使用商家端登录')
...
async def wx_login(req: WxLoginRequest, request: Request) -> dict[str, Any]:
    ...
    if not settings.auth_configured:
        raise HTTPException(status_code=503, detail='微信登录未配置：请在 .env 设置 WECHAT_APPID / WECHAT_SECRET')
    try:
    ...
    return {'token': token, 'user_id': uid, 'openid': openid, 'unionid': info.get('unionid'), 'nickname': profile.get('nickname') or '微信用户', 'is_new': is_new, 'expires_in': settings.jwt_expire_minutes * 60, 'token_type': 'Bearer'}
...
async def phone_login(req: PhoneLoginRequest) -> dict[str, Any]:
    ...
    _reject_c_end(uid)
    profile = security.get_user_profile(uid) or {}
    return {'token': token, 'user_id': uid, 'openid': uid, 'nickname': profile.get('nickname') or req.phone, 'phone': req.phone, 'is_new': is_new, 'expires_in': settings.jwt_expire_minutes * 60, 'token_type': 'Bearer'}
...
async def wx_bind(req: WxBindRequest, request: Request) -> dict[str, Any]:
    ...
    if not settings.auth_configured:
        raise HTTPException(status_code=503, detail='微信绑定未配置：请在 .env 设置 WECHAT_APPID / WECHAT_SECRET')
    try:
        info = await asyncio.to_thread(wx_code2session, req.code)
    except httpx.HTTPError as exc:
...
async def register(req: RegisterRequest, request: Request) -> dict[str, Any]:
    ...
    profile = security.get_user_profile(uid) or {}
    return {'token': token, 'user_id': uid, 'openid': uid, 'nickname': profile.get('nickname', req.username), 'expires_in': settings.jwt_expire_minutes * 60, 'token_type': 'Bearer'}
...
@router.post('/auth/merchant-register')
async def merchant_register(req: MerchantRegisterRequest, request: Request) -> dict[str, Any]:
    """商家独立注册（三端架构阶段2）：手机号全局唯一，注册即 role=merchant。

    手机号已被任意角色（user/merchant/admin）占用时返回 409；密码过短返回 422（pydantic）或 400。
    """
    _check_rate(f'auth:{_client_ip(request)}', settings.rate_limit_auth_per_minute)
    try:
        uid, token = security.register_merchant(req.phone, req.password, req.shop_name)
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    profile = security.get_user_profile(uid) or {}
    return {'token': token, 'user_id': uid, 'openid': uid, 'phone': req.phone, 'nickname': profile.get('nickname', ''), 'role': 'merchant', 'expires_in': settings.jwt_expire_minutes * 60, 'token_type': 'Bearer'}

@router.post('/auth/merchant-login')
async def merchant_login(req: LoginRequest, request: Request) -> dict[str, Any]:
    """商家端登录：校验凭据并要求 role=merchant，否则 403（C 端用户/管理员无法登录商家端）。

    登录账号为注册时填写的手机号（username 字段即手机号）。
    """
    _check_rate(f'auth:{_client_ip(request)}', settings.rate_limit_auth_per_minute)
    token = security.login_user(req.username, req.password)
    if not token:
    ...
    uid = security.verify_token(token)
    if security.get_user_role(uid) != 'merchant':
        raise HTTPException(status_code=403, detail='该账号不是商家，无法进入商家端')
    profile = security.get_user_profile(uid) or {}
    return {'token': token, 'user_id': uid, 'openid': uid, 'phone': req.username, 'nickname': profile.get('nickname', ''), 'role': 'merchant', 'expires_in': settings.jwt_expire_minutes * 60, 'token_type': 'Bearer'}
