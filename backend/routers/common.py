...
class RateLimiter(ABC):
    """限流抽象：窗口内请求数 ≥ limit 则拒绝。实现可替换为内存 / Redis（接口不变）。"""
...
class RedisRateLimiter(RateLimiter):
    """基于 Redis 有序集合的滑动窗口限流，多 worker 共享计数。

    使用同步客户端（redis-py）以兼容现有同步 _check_rate 调用点；
    Redis 不可用时降级放行并告警，不阻断业务。
    """

    def __init__(self) -> None:
        import redis
        if not settings.redis_url:
            raise RuntimeError('RedisRateLimiter 需要配置 redis_url')
        self._r = redis.Redis.from_url(settings.redis_url, socket_connect_timeout=settings.redis_socket_timeout, decode_responses=True)
    ...
    def allow(self, key: str, limit: int, window: float=60.0) -> bool:
        ...
        try:
            ...
            logger.exception('[ratelimit] Redis 限流失效，降级放行')
            return True

def _build_limiter() -> RateLimiter:
    """按配置选择限流实现：配了 redis_url 优先 Redis，否则内存。"""
    if settings.redis_url:
        try:
            return RedisRateLimiter()
        except Exception:
            logger.warning('[ratelimit] Redis 限流器初始化失败，回退内存限流')
...
class OrderCreateRequest(BaseModel):
    ...
    delivery_location: dict[str, Any] | None = Field(None, description='配送位置（地图选点）：{lat, lng, address}，与收货地址分开存储')

class PayRequest(BaseModel):
    order_id: str = Field(..., min_length=1, max_length=64)
    method: str | None = Field(None, description='wechat_native(微信扫码) | alipay(支付宝) | 留空=用配置默认渠道(sandbox)')
...
class ImageGenRequest(BaseModel):
    prompt: str = Field(..., min_length=1, max_length=2000, description='生图提示词：DIY 方案的 effect_prompt 或自定义描述')
...
def _derive_flowers(p: dict[str, Any]) -> list[str]:
    ...
    desc = p.get('desc', '')
    head = re.split('[，,。]', desc)[0] if desc else ''
    out: list[str] = []
    for token in re.split('[+、/]|\\s+', head):
        t = re.sub('^[\\d\\s支朵枝盆棵个]*', '', token).strip()
...
async def _plan_full(p: dict[str, Any]) -> dict[str, Any]:
    """方案详情（商品详情页 / DIY 详情直链兜底）。"""
    base = await _plan_card(p)
...
def _shop_card(s: dict[str, Any], location: dict[str, float] | None=None) -> dict[str, Any]:
    ...
    return {'id': s['shop_id'], 'name': s['name'], 'rating': str(s.get('rating', '4.8')), 'dist': f'{d:.1f}km' if isinstance(d, float) else f'{d}km', 'eta': '配送约30分钟', 'price_range': s.get('price_range', ''), 'min_delivery': float(s.get('min_delivery') or (int(float(s.get('price_range', '0').split('-')[0])) // 10 * 10 if s.get('price_range') and s['price_range'].split('-')[0].strip().isdigit() else 30)), 'delivery_fee': float(s.get('delivery_fee') or (3 if float(s.get('distance_km') or 1) <= 1 else 5 if float(s.get('distance_km') or 1) <= 2.5 else 8)), 'image': s.get('image') or ''}
...
def _shop_menu_item(p: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    return {'id': p['plan_id'], 'name': p['name'], 'price': p['price'], 'desc': p.get('desc', ''), 'tags': p.get('tags', []), 'style': p.get('style', ''), 'image': p.get('effect_image_url'), 'label': _plan_label(p), 'sales': int(p.get('sold', 0))}

async def _shop_full(s: dict[str, Any]) -> dict[str, Any]:
    ...
    return {'id': s['shop_id'], 'name': s['name'], 'rating': str(s.get('rating', '4.8')), 'status': '营业中', 'distance_km': float(s.get('distance_km') or 0), 'intro': s.get('intro', '专注鲜花定制与同城速递，包装精致、准时送达。'), 'sales': int(s.get('sales', 0)), 'min_delivery': float(s.get('min_delivery') or 30), 'delivery_fee': float(s.get('delivery_fee') or 5), 'delivery_time': s.get('delivery_time') or '30分钟', 'hours': s.get('hours') or '09:00 - 21:00', 'address': s.get('address') or '深圳市盐田区海景路 1 号（示例地址）', 'notice': s.get('notice') or s.get('intro', '专注鲜花定制与同城速递，包装精致、准时送达。'), 'image': s.get('image') or '', 'cover': s.get('cover') or '', 'logo': s.get('logo') or '', 'menu': menu, 'recommend': [{'id': p['plan_id'], 'name': p['name'], 'price': p['price']} for p in plans]}
...
async def _merchant_scope(request: Request) -> tuple[str, list[str]]:
    ...
    shop_ids = await catalog_store.merchant_shop_ids(uid)
    return (uid, shop_ids)

def _require_shop_in_scope(shop_id: str, scope: list[str] | None) -> None:
