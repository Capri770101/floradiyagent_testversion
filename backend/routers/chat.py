...
from backend.storage.object_store import read_generated
...
async def chat_stream(req: ChatRequest, request: Request) -> StreamingResponse:
    ...
    return StreamingResponse(await event_generator(), media_type='text/event-stream', headers={'Cache-Control': 'no-cache', 'Connection': 'keep-alive', 'X-Accel-Buffering': 'no'})
...
async def get_task(task_id: str, request: Request) -> dict[str, Any]:
    ...
    return await tasks.get_image_task(task_id)
...
async def generate_image(req: ImageGenRequest, request: Request) -> dict[str, Any]:
    ...
    task_id = await tasks.create_image_task(req.prompt)
    return {'task_id': task_id, 'status': 'submitted', 'poll': f'/tasks/{task_id}'}

@router.get('/generated/{filename}')
async def serve_generated(filename: str) -> Response:
    """托管生图产物（api2img 中转商 base64 落盘 / 占位图等）。

    文件名做严格白名单校验防目录穿越；读取经对象存储抽象层，local 模式读本地磁盘，
    s3/oss 模式读对象存储，统一返回图片字节流。
    """
    if not re.fullmatch('[A-Za-z0-9_.\\-]+', filename):
        raise HTTPException(status_code=400, detail='非法文件名')
    data = read_generated(filename)
    if data is None:
...
async def reset(req: ResetRequest, request: Request) -> dict[str, Any]:
    ...
    ok = await mem_store.reset_session(user_id, req.conversation_id)
...
async def list_conversations(request: Request, user_id: str | None=None) -> dict[str, Any]:
    ...
    convs = await mem_store.list_conversations(uid)
...
async def get_conversation_messages(conv_id: str, request: Request, user_id: str | None=None) -> dict[str, Any]:
    ...
    uid = await resolve_uid(request, user_id)
    if uid:
        conv = await mem_store.get_conversation(conv_id)
        if not conv or conv.get('user_id') != uid:
            raise HTTPException(status_code=404, detail='会话不存在')
    msgs = await mem_store.load_display_messages(conv_id)
...
async def rename_conversation(conv_id: str, req: RenameConvRequest, request: Request) -> dict[str, Any]:
    ...
    if uid:
        ...
        if not conv or conv.get('user_id') != uid:
            raise HTTPException(status_code=404, detail='会话不存在')
    ok = await mem_store.rename_conversation(conv_id, req.title)
    if not ok:
        raise HTTPException(status_code=404, detail='会话不存在')
    return {'ok': True}
...
async def delete_conversation(conv_id: str, request: Request, user_id: str | None=None) -> dict[str, Any]:
    ...
    if uid:
        ...
        if not conv or conv.get('user_id') != uid:
            raise HTTPException(status_code=404, detail='会话不存在')
    ok = await mem_store.delete_conversation(conv_id)
    if not ok:
        raise HTTPException(status_code=404, detail='会话不存在')
    return {'ok': True}
