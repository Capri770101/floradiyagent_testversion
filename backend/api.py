...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    init_db()
    try:
        await db_async.init_db_async()
    except Exception:
        logger.warning('PG 初始化失败（不影响 sqlite 默认路径）：%s', exc_info=True)
    ...
    if settings.app_env == 'prod':
        ...
        if missing:
            raise RuntimeError('生产环境（APP_ENV=prod）启动校验失败，缺少: ' + ', '.join(missing) + '。请先在 misc/.env 配置后再启动。')
    ...
    if not settings.auth_required:
        logger.warning('⚠️ AUTH_REQUIRED=false（开发态）：/chat 与 /image/generate 可用任意 user_id 直连，生产环境务必设 AUTH_REQUIRED=true 并自设 JWT_SECRET（否则令牌可被伪造）')
    if settings.payment_provider != 'sandbox' and (not settings.payment_configured):
        logger.warning('⚠️ PAYMENT_PROVIDER=%s 但未完整配置凭据，/pay 将返回 400；如需端到端验证请先用 PAYMENT_PROVIDER=sandbox', settings.payment_provider)
    logger.info('=' * 64)
    logger.info('%s 启动 | LLM=%s | 生图=%s | 鉴权=%s | 数据源=%s', settings.app_name, 'live' if settings.llm_enabled else 'mock', 'live' if settings.image_enabled else 'mock', 'required' if settings.auth_required else 'dev', settings.data_source)
...
app.add_middleware(CORSMiddleware, allow_origins=settings.cors_origins, allow_credentials=_allow_credentials, allow_methods=['*'], allow_headers=['*'])
...
async def request_logging(request: Request, call_next: Any) -> Any:
    ...
    try:
        observability.record_request(response.status_code, elapsed)
    except Exception:
        pass
    return response
...
async def metrics_endpoint() -> PlainTextResponse:
    """P7 可观测性：Prometheus 格式指标（QPS / P95 耗时 / 错误率 / LLM 成本 / 生图成功率 / 限流命中）。"""
