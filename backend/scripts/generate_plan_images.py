...
def build_prompt(name: str, desc: str, tags: str) -> str:
    ...
    return f'鲜花电商产品图：{name}。{desc}关键词：{tag_txt}。浅米色纯净背景，柔和自然光，居中构图，花材新鲜饱满带水珠，高清写实商业摄影，无文字无水印。'

def _cleanup_old(old_url: str, new_url: str) -> None:
    """扩展名变化时清理旧落盘文件（优先走对象存储删除，local 模式回退本地删文件）。"""
    if old_url and old_url != new_url and old_url.startswith('/generated/'):
        key = Path(old_url).name
        try:
            from backend.storage.object_store import get_object_store
            get_object_store().delete(key, namespace='generated')
        except Exception:
            old_file = tasks._ensure_generated_dir() / key
            if old_file.exists() and old_file.name != Path(new_url).name:
                old_file.unlink(missing_ok=True)

def gen_plans(conn, only: set[str]) -> None:
    rows = conn.execute('SELECT id, name, price, desc, tags, effect_image_url FROM plans ORDER BY id').fetchall()
    if only:
    ...
    for r in rows:
        ...
        try:
            ...
            continue
        with conn:
            conn.execute('UPDATE plans SET effect_image_url=? WHERE id=?', (new_url, pid))
...
def gen_shops(conn) -> None:
    ...
    rows = conn.execute('SELECT id, name, intro, address, cover, logo FROM shops ORDER BY id').fetchall()
    print(f'待出图店铺 {len(rows)} 家')
    for s in rows:
        sid = s['id']
        base = f"花店「{s['name']}」店面实拍：{s['intro'] or '温馨花艺工作室'}，位于{s['address'] or '街边'}。"
        jobs = [('cover', '1344x768', base + '宽幅门头横幅视角，木质招牌，门口陈列鲜花绿植，暖色灯光，清新文艺氛围，高清写实商业摄影，无文字无水印。'), ('logo', '1024x1024', base + '方形门头特写构图，招牌旁悬挂花束装饰，柔和自然光，温馨精致，高清写实摄影，无文字无水印。')]
        for field, size, prompt in jobs:
            old_url = s[field] or ''
            try:
                new_url = tasks._image_client_submit_zhipu(prompt, f'shop_{sid}_{field}', size=size)
            except Exception as exc:
                print(f'  x {sid} {field}: 出图失败（保留原图 {old_url}）：{exc}')
                continue
            with conn:
                if field == 'cover':
                    conn.execute('UPDATE shops SET cover=?, image=? WHERE id=?', (new_url, new_url, sid))
                else:
