...
from routers.common import _plan_card, _shop_card, resolve_uid
from storage import recommend as rec

router = APIRouter(prefix="/recommend", tags=["recommend"])


@router.get("/plans")
async def recommend_plans_endpoint(
    request: Request,
    lat: float | None = Query(None, ge=-90, le=90),
    lng: float | None = Query(None, ge=-180, le=180),
    limit: int = Query(6, ge=1, le=20),
    style: str | None = Query(None, max_length=20),
    user_id: str | None = None,
) -> dict[str, Any]:
    """个性化方案推荐：定位 + 偏好 + 热度；style 传值时同风格方案加权。"""
    uid = await resolve_uid(request, user_id)
    items = rec.recommend_plans(uid, lat, lng, limit, style)
    return {"items": [_plan_card(p) for p in items], "total": len(items)}


@router.get("/shops")
async def recommend_shops_endpoint(
    request: Request,
    lat: float | None = Query(None, ge=-90, le=90),
    lng: float | None = Query(None, ge=-180, le=180),
    limit: int = Query(6, ge=1, le=20),
    shop_id: str | None = Query(None, max_length=64),
    user_id: str | None = None,
) -> dict[str, Any]:
    """附近同类店铺推荐：排除自身（shop_id），同价位带加权。"""
    uid = await resolve_uid(request, user_id)
    location = {"lat": lat, "lng": lng} if lat is not None and lng is not None else None
    items = rec.recommend_shops(uid, lat, lng, limit, shop_id)
    return {"items": [_shop_card(s, location) for s in items], "total": len(items)}


@router.get("/signature")
async def recommend_signature_endpoint(
    lat: float | None = Query(None, ge=-90, le=90),
    lng: float | None = Query(None, ge=-180, le=180),
    limit: int = Query(3, ge=1, le=10),
) -> dict[str, Any]:
    """当季臻选：策展式推荐（角标气质 + 热度 + 距离），首页 Signature Collection 位。"""
    items = rec.recommend_signature(lat, lng, limit)
    out = []
    for p in items:
        card = _plan_card(p)
