...
from routers.common import _require_admin, resolve_uid
from storage import notify as notify_store
...
async def list_notifications_endpoint(
    ...
    items = await asyncio.to_thread(
        notify_store.list_notifications, uid, type.strip(), is_read,
...
async def unread_count_endpoint(request: Request) -> dict[str, Any]:
    ...
    if not uid:
        raise HTTPException(status_code=401, detail="缺少用户身份")
    return {"unread": await asyncio.to_thread(notify_store.count_unread, uid)}
...
async def notification_detail_endpoint(notification_id: str, request: Request) -> dict[str, Any]:
    ...
    if not uid:
        raise HTTPException(status_code=401, detail="缺少用户身份")
    item = await asyncio.to_thread(notify_store.get_notification, notification_id, uid)
    if not item:
...
async def mark_read_endpoint(req: MarkReadRequest, request: Request) -> dict[str, Any]:
    ...
    if not uid:
        raise HTTPException(status_code=401, detail="缺少用户身份")
    n = await asyncio.to_thread(notify_store.mark_read, uid, req.ids, req.all)
...
async def admin_send_notification(
    req: AnnouncementWriteRequest, request: Request
) -> dict[str, Any]:
    ...
    n = await asyncio.to_thread(
        notify_store.broadcast,
        req.title, req.body, req.ntype, "", "", req.user_ids,
    )
