...
import { IconStore, IconPlus, IconHeart } from './icons'
...
// compact：推荐位横滑用——名称单行截断，保证每张卡高度完全一致
// fav/onFav：推荐位收藏动线（仅 compact 传入时渲染心形按钮）
// dist：当季臻选等按距离策展的卡片，在店内行下展示「距你 x.xkm」
export default function ProductCard({ p, onOpen, onAdd, compact = false, fav, onFav, dist }) {
  return (
    <div
      role="button"
      tabIndex={0}
      ...
      className="press relative flex cursor-pointer flex-col overflow-hidden rounded-[4px] border border-line bg-white"
    >
      {compact && onFav && (
        <button
          onClick={(e) => {
          ...
          className={`absolute right-2.5 top-2.5 z-10 flex h-8 w-8 items-center justify-center rounded-full ${
            fav ? 'bg-burgundy text-white' : 'bg-white/85 text-stone'
          }`}
        >
          <IconHeart width={15} height={15} filled={fav} />
        </button>
      )}
      <CardImage p={p} />
      <div className={`flex flex-1 flex-col ${compact ? 'px-3 pb-3 pt-2.5' : 'px-4 pb-4 pt-3.5'}`}>
        <p
          className={`font-serif-cn font-normal leading-snug text-ink ${
            compact ? 'truncate text-[15px]' : 'text-[19px]'
          }`}
        >
          {p.name}
        </p>
        ...
        {!compact && dist != null && (
          <p className="mt-1 text-[10px] text-stone">距你 {Number(dist).toFixed(1)}km</p>
        )}
        <div className={`mt-auto flex items-center justify-between ${compact ? 'pt-2.5' : 'pt-3'}`}>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-[2px] bg-dark text-[#FAF8F5] ${
              compact ? 'px-3 py-1.5 text-[10px] tracking-[0.5px]' : 'px-4 py-2 text-[11px] font-medium tracking-[1px]'
            }`}
          >
