...
def _create_order(client, token, shop='S001'):
    r = client.post('/orders', headers=_h(token), json={'items': [{'plan_id': 'P001', 'name': '测试花束', 'price': 99, 'qty': 1, 'shop': shop}]})
    assert r.status_code == 200, r.text
...
def test_merchant_logistics_node_creates_logistics_notify(client):
    ...
    assert asyncio.run(catalog.merchant_bind(muid, 'S001'))
...
def test_aftersale_and_review_hooks(client):
    ...
    assert r.status_code == 200, r.text
    r = client.post(f'/orders/{oid}/aftersale', headers=_h(token), json={'reason': '花材枯萎', 'type': 'refund'})
    assert r.status_code == 200, r.text
    ...
    asyncio.run(catalog.merchant_bind(muid, 'S001'))
...
def test_admin_broadcast_announcement(client):
    ...
    r = client.post('/admin/notifications', headers=_h(atok), json={'title': '仅买家可见', 'body': '', 'user_ids': [buid]})
    assert r.status_code == 200 and r.json()['sent'] == 1
    assert len(_notifies(client, btok, type='announcement', is_read=0)) == 1
    r = client.post('/admin/notifications', headers=_h(btok), json={'title': '越权', 'body': ''})
    assert r.status_code == 403

def test_list_filter_pagination_and_mark_read(client):
    token, uid = _register(client, 'noti_buyer5')
    for i in range(5):
        asyncio.run(notify_store.create_notification(uid, 'system', f'消息{i}', f'正文{i}'))
...
def test_detail_ownership_and_cross_user_mark_read(client):
    ...
    asyncio.run(notify_store.create_notification(ua, 'system', 'A的消息', '仅A可见'))
