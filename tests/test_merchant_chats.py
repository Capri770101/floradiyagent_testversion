...
def _bind_merchant(client, username, shop='S001'):
    ...
    assert asyncio.run(catalog.merchant_bind(uid, shop))
    return (token, uid)
...
def test_merchant_chat_with_user_creates(client):
    ...
    r = client.post('/merchant/chats/with-user', json={'user_id': uid, 'shop_id': 'S001'}, headers=_h(m_token))
    assert r.status_code == 200, r.text
    chat = r.json()['chat']
    assert chat['user_id'] == uid
    assert chat['shop_id'] == 'S001'
    r2 = client.post('/merchant/chats/with-user', json={'user_id': uid, 'shop_id': 'S001'}, headers=_h(m_token))
...
def test_merchant_chat_with_user_scope_forbidden(client):
    ...
    r = client.post('/merchant/chats/with-user', json={'user_id': uid, 'shop_id': 'S002'}, headers=_h(m_s1))
    assert r.status_code == 403
...
def test_merchant_review_reply(client):
    ...
    r = client.post(f'/merchant/reviews/{rev_id}/reply', json={'reply': '感谢您的评价，欢迎再来！'}, headers=_h(m_token))
    assert r.status_code == 200, r.text
...
def test_merchant_review_reply_not_own_shop(client):
    ...
    r = client.post(f'/merchant/reviews/{rev_id}/reply', json={'reply': '越权'}, headers=_h(m_s1))
...
def test_merchant_review_reply_requires_merchant(client):
    ...
    r = client.post(f'/merchant/reviews/{rev_id}/reply', json={'reply': '普通用户'}, headers=_h(token))
    assert r.status_code == 403
