...
def _add_addr(client, token, name='木木', addr='深圳市南山区xx路1号', default=False):
    r = client.post('/addresses', headers={'Authorization': f'Bearer {token}'}, json={'name': name, 'phone': '13800000000', 'address': addr, 'is_default': default})
    assert r.status_code == 200, r.text
...
def test_address_crud_and_first_default(client):
    ...
    assert a1['is_default'] == 1
    a2 = _add_addr(client, token, addr='广州市天河区xx路2号')
    assert a2['is_default'] == 0
    r = client.put(f"/addresses/{a2['id']}", headers={'Authorization': f'Bearer {token}'}, json={'is_default': True})
    ...
    r = client.put(f"/addresses/{a1['id']}", headers={'Authorization': f'Bearer {token}'}, json={'phone': '13900000000'})
...
def test_address_isolated_by_user(client):
    ...
    assert items == []
    r = client.put(f"/addresses/{a['id']}", headers={'Authorization': f'Bearer {t2}'}, json={'phone': '13900000000'})
    assert r.status_code == 404

def test_order_with_address_id(client):
    token = _register(client, 'addr_c')
    a = _add_addr(client, token)
    r = client.post('/orders', headers={'Authorization': f'Bearer {token}'}, json={'items': [{'plan_id': 'P001', 'name': '测试花束', 'price': 99, 'qty': 1, 'shop': 'S001'}], 'address_id': a['id']})
    assert r.status_code == 200, r.text
    o = r.json()['order']
...
def test_favorite_toggle_and_list(client):
    ...
    r = client.post('/favorites', headers={'Authorization': f'Bearer {token}'}, json={'plan_id': 'P001'})
    assert r.status_code == 200
    r = client.post('/favorites', headers={'Authorization': f'Bearer {token}'}, json={'plan_id': 'P001'})
    assert r.status_code == 200
