...
def test_patch_updates_recipient_and_persists(client):
    ...
    r = client.patch(f'/orders/{oid}', headers={'Authorization': f'Bearer {token}'}, json={'recipient': {'name': '木木', 'phone': '13800000000', 'address': '深圳市盐田区xx路1号'}, 'delivery': '今天 18:00–20:00', 'note': '请放门口'})
    assert r.status_code == 200, r.text
    o = r.json()['order']
...
def test_patch_partial_only_updates_given_fields(client):
    ...
    oid = _create_order(client, token)
    client.patch(f'/orders/{oid}', headers={'Authorization': f'Bearer {token}'}, json={'recipient': {'name': '仅改名字'}})
...
def test_patch_other_user_order_is_forbidden(client):
    ...
    r = client.patch(f'/orders/{oid}', headers={'Authorization': f'Bearer {token_b}'}, json={'note': '偷改'})
