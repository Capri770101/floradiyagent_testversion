...
def _register(client, username):
    r = client.post('/auth/register', json={'username': username, 'password': 'secret123', 'nickname': '超时测试'})
...
def _create_order(client, token, price=99):
    r = client.post('/orders', headers=_h(token), json={'items': [{'plan_id': 'P001', 'name': '测试花束', 'price': price, 'qty': 1, 'shop': 'S001'}]})
    assert r.status_code == 200, r.text
    return r.json()['order']
