...
def test_merchant_register_success_issues_jwt(client: TestClient) -> None:
    ...
    r = client.post('/auth/merchant-register', json={'phone': '13800000001', 'password': 'secret1', 'shop_name': '测试花店'})
...
def test_merchant_register_phone_taken_by_user_conflict(client: TestClient) -> None:
    ...
    assert client.post('/auth/register', json={'username': 'u_phone_owner', 'password': 'secret1'}).status_code == 200
    client.post('/auth/phone-code', json={'phone': '13800000003'})
    assert client.post('/auth/phone-login', json={'phone': '13800000003', 'code': '123456'}).status_code == 200
    r = client.post('/auth/merchant-register', json={'phone': '13800000003', 'password': 'secret1'})
    assert r.status_code == 409
...
def test_merchant_login_success_and_role_gate(client: TestClient) -> None:
    ...
    r = client.post('/auth/merchant-register', json={'phone': '13800000004', 'password': 'secret1'})
    mid = r.json()['user_id']
    r = client.post('/auth/merchant-login', json={'username': '13800000004', 'password': 'secret1'})
    assert r.status_code == 200, r.text
    assert r.json()['role'] == 'merchant'
    assert security.verify_token(r.json()['token']) == mid
    assert client.post('/auth/merchant-login', json={'username': '13800000004', 'password': 'wrong1'}).status_code == 401
    client.post('/auth/register', json={'username': 'm_gate_user', 'password': 'secret1'})
    assert client.post('/auth/merchant-login', json={'username': 'm_gate_user', 'password': 'secret1'}).status_code == 403
...
def test_merchant_login_rejects_admin(client: TestClient) -> None:
    ...
    uid = security.verify_token(client.post('/auth/login', json={'username': 'm_admin_seed', 'password': 'secret1'}).json()['token'])
    security.set_user_role(uid, 'admin')
    r = client.post('/auth/merchant-login', json={'username': 'm_admin_seed', 'password': 'secret1'})
    assert r.status_code == 403

def test_c_end_rejects_merchant_login_and_register(client: TestClient) -> None:
    """商家手机号走 C 端登录被 403；同名 C 端注册被 409（角色隔离）。"""
    client.post('/auth/merchant-register', json={'phone': '13800000005', 'password': 'secret1'})
    ...
    r = client.post('/auth/register', json={'username': '13800000005', 'password': 'secret1'})
    assert r.status_code == 409

def test_merchant_token_can_call_merchant_api(client: TestClient) -> None:
    """商家 JWT 可调用受 _require_merchant 守护的商家端接口（端到端闭环）。"""
    r = client.post('/auth/merchant-register', json={'phone': '13800000006', 'password': 'secret1'})
    token = r.json()['token']
...
def test_merchant_aftersales_scoped(client: TestClient) -> None:
    ...
    r = client.post('/auth/merchant-register', json={'phone': '13800000007', 'password': 'secret1'})
    token = r.json()['token']
    h = {'Authorization': f'Bearer {token}'}
