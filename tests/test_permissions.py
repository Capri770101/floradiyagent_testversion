...
def test_admin_role_allows_crud(client):
    ...
    r = client.post('/admin/plans', headers=_h(token), json={'plan_id': 'P999', 'name': '权限测试方案', 'price': 66})
    assert r.status_code == 200
    pid = r.json()['plan']['plan_id']
    r = client.put(f'/admin/plans/{pid}', headers=_h(token), json={'price': 88})
...
def test_review_requires_done_order_and_owner(client):
    ...
    r2 = client.post('/orders', headers=_h(token), json={'items': [{'plan_id': 'P001', 'name': 'x', 'price': 10, 'qty': 1, 'shop': 'S001'}]})
