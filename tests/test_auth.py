...
def test_wx_login_unconfigured_returns_503(client: TestClient) -> None:
    ...
    with patch.object(security.settings, "wechat_appid", ""), \
         patch.object(security.settings, "wechat_secret", ""):
...
def test_wx_login_success_issues_verifiable_jwt(client: TestClient) -> None:
    ...
    with patch.object(security.settings, "wechat_appid", "appid"), \
         ...
         patch("backend.routers.auth.wx_code2session", return_value=fake):
        r = client.post("/auth/wx-login", json={"code": "abc"})
        assert r.status_code == 200
        body = r.json()
        assert body["openid"] == "oABC123"
        # 签发的 token 可被正确校验，且 openid 一致
