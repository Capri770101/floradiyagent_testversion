...
def _neutralize_heat():
    """seed_catalog 热度为演示值可叠加 hash() 噪声 → 需中和以保证确定性测试。"""
    ...
    if dba.dialect() == 'postgresql':
        ...
        async def _n():
            async with dba.transaction() as c:
                await c.execute('UPDATE plans SET rating=4.5, sold=100')
                await c.execute('UPDATE shops SET rating=4.5, sales=100')
        _run_async(_n())
        return
...
def test_extract_preferences_from_favorites_and_orders(client):
    ...
    asyncio.run(catalog.merchant_bind(uid, 'S001'))
    asyncio.run(commerce.add_favorite(uid, 'P001'))
    order = asyncio.run(commerce.create_order(user_id=uid, items=[{'plan_id': 'P001', 'qty': 1, 'shop': 'S001', 'name': '康乃馨感恩花束'}], recipient={'name': 'A', 'phone': '1', 'address': 'x'}))
    assert order['order_id']
    prefs = asyncio.run(recommend.extract_preferences(uid))
    assert prefs['styles'].get('韩式', 0) >= 2
    assert '温馨' in prefs['tags']
    assert prefs['bands'].get(1)
    assert prefs['shops'].get('S001') == 1
    empty = asyncio.run(recommend.extract_preferences(None))
...
def test_fusion_ordering_favors_preferred_style(client):
    ...
    _neutralize_heat()
    asyncio.run(commerce.add_favorite(uid, 'P001'))
    items = asyncio.run(recommend.recommend_plans(uid, limit=10))
...
def test_location_weights_nearer_first():
    ...
    _neutralize_heat()
    lat, lng = (22.56, 114.242)
    items = asyncio.run(recommend.recommend_plans(None, lat, lng, limit=6))
    ranked = [p['plan_id'] for p in items]
    assert ranked
...
def test_fallback_without_location_and_prefs():
    ...
    items = asyncio.run(recommend.recommend_plans(None, limit=5))
    assert len(items) == 5
...
def test_recommend_shops_excludes_self_and_same_range_boost():
    ...
    _neutralize_heat()
    items = asyncio.run(recommend.recommend_shops(None, limit=10, shop_id='S001'))
    ...
    near = asyncio.run(recommend.recommend_shops(None, 22.56, 114.242, limit=3, shop_id='S001'))
...
def test_recommend_style_param_boosts_same_style():
    ...
    _neutralize_heat()
    items = asyncio.run(recommend.recommend_plans(None, limit=10, style='韩式'))
    ranked = [p['plan_id'] for p in items]
    assert ranked
    assert ranked[0] in ('P001', 'P013')
...
def test_style_groups_normalize_alias_styles():
    ...
    assert _style_group('') == ''
    _neutralize_heat()
    items = asyncio.run(recommend.recommend_plans(None, limit=10, style='北欧'))
    assert items and items[0]['style'] in ('简约', 'ins')
...
def test_recommend_weights_operations_config():
    ...
    base = asyncio.run(config_store.public_config())
    assert set(base['recommend_weights']) == {'w_distance', 'w_pref', 'w_heat'}
    try:
        out = asyncio.run(config_store.update_operations({'recommend_weights': {'w_distance': 0.9}}))
        assert out['recommend_weights']['w_distance'] == 0.9
        assert out['recommend_weights']['w_pref'] == 0.4
        with pytest.raises(ValueError):
            asyncio.run(config_store.update_operations({'recommend_weights': {'w_distance': 1.5}}))
    finally:
        asyncio.run(config_store.update_operations({'recommend_weights': {'w_distance': base['recommend_weights']['w_distance']}}))

def test_recommend_signature_prefers_premium_without_location():
    """当季臻选：无定位时角标气质主导（Premium ≥300 元居前），不依赖用户画像。"""
    _neutralize_heat()
    items = asyncio.run(recommend.recommend_signature(limit=6))
    assert items and len(items) == 6
    assert all(p['price'] >= 300 for p in items[:4])
    assert items[0]['dist_km'] is None
    uid = _uid(9)
    from backend.storage import commerce
    asyncio.run(commerce.add_favorite(uid, 'P001'))
    a = asyncio.run(recommend.recommend_signature(limit=6))
    b = asyncio.run(recommend.recommend_signature(limit=6))
...
def test_recommend_signature_distance_tiebreaks_same_label():
    ...
    3) dist_km 有值且合理。"""
    _neutralize_heat()
    items = asyncio.run(recommend.recommend_signature(22.56, 114.242, limit=20))
    ranked = [p['plan_id'] for p in items]
