...
def test_report_requires_login(client):
    r = client.post("/reports", json={
        "target_type": "plan", "target_id": "P001", "reason": "违规内容",
    })
...
def test_report_create_and_invalid_type(client):
    ...
    r = client.post("/reports", json={
        "target_type": "plan", "target_id": "P001", "reason": "图片违规", "content": "涉嫌违规图",
    }, headers=_h(token))
    assert r.status_code == 200, r.text
    item = r.json()["report"]
    ...
    # 非法 target_type
    r = client.post("/reports", json={
        "target_type": "bogus", "target_id": "P001", "reason": "x",
    }, headers=_h(token))
...
def test_report_shop_and_list_scoped(client):
    ...
    client.post("/reports", json={
        "target_type": "shop", "target_id": "S001", "reason": "虚假宣传",
    }, headers=_h(token))
    # 非 admin 查列表 → 403
...
def test_report_admin_handle_banned_takes_down_plan(client):
    ...
    r = client.post("/reports", json={
        "target_type": "plan", "target_id": "P001", "reason": "违禁品",
    }, headers=_h(token))
    ...
    r2 = client.post("/reports", json={
        "target_type": "shop", "target_id": "S001", "reason": "x",
    }, headers=_h(token))
...
def test_report_handle_rejected_and_review_target(client):
    ...
    # 评价目标：随便用一个不存在的 review id 走 rejected（不下架）
    r = client.post("/reports", json={
        "target_type": "review", "target_id": "RV_NOPE", "reason": "辱骂",
    }, headers=_h(token))
