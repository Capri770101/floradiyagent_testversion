...
def _create_and_pay(client, token, shop='S001', price=99):
    r = client.post('/orders', headers=_h(token), json={'items': [{'plan_id': 'P001', 'name': '测试花束', 'price': price, 'qty': 1, 'shop': shop}]})
    assert r.status_code == 200, r.text
...
def test_banned_merchant_token_invalidated_immediately(client):
    ...
    assert asyncio.run(catalog.merchant_bind(uid, 'S001'))
...
def test_aftersale_flow(client):
    ...
    oid = _create_and_pay(client, user_t)
    r = client.post(f'/orders/{oid}/aftersale', json={'type': 'refund', 'reason': '不想要了', 'description': '花材品质不符'}, headers=_h(user_t))
    assert r.status_code == 200, r.text
    as_id = r.json()['aftersale']['id']
    r = client.post('/orders', headers=_h(user_t), json={'items': [{'plan_id': 'P001', 'name': 'x', 'price': 1, 'qty': 1, 'shop': 'S001'}]})
...
def test_aftersale_refund_links_payment(client):
    ...
    if dba.dialect() == 'postgresql':
        ...
        row = get_conn().execute('SELECT status FROM payments WHERE order_id=?', (oid,)).fetchone()
    assert row and row['status'] == 'refunded'

def test_merchant_apply_flow(client):
    admin_t = _admin_token(client)
    user_t, uid = _register(client, 'adm_g')
    r = client.post('/merchant/apply', json={'shop_name': '测试花店', 'contact_name': '张三', 'contact_phone': '13800001111', 'license_no': 'L-001', 'intro': '专注测试'}, headers=_h(user_t))
    assert r.status_code == 200, r.text
    ...
    assert asyncio.run(catalog.merchant_shops(uid))
    r = client.get('/admin/merchants', headers=_h(admin_t))
    assert r.status_code == 200
    assert any(m['user_id'] == uid for m in r.json()['merchants'])
    for s in asyncio.run(catalog.merchant_shops(uid)):
        asyncio.run(catalog.delete_shop(s['id']))
...
def test_merchant_apply_reject(client):
    ...
    app_id = r.json()['application']['id']
    r = client.post(f'/admin/merchant-applications/{app_id}/reject', json={'note': '执照不清晰'}, headers=_h(admin_t))
    assert r.status_code == 200
...
def _create_review(client, token, username):
    ...
    assert r.status_code == 200, r.text
    r = client.post('/reviews', json={'order_id': oid, 'rating': 4, 'content': '测试评价内容'}, headers=_h(token))
    assert r.status_code == 200, r.text
...
def test_operations_config_flow(client):
    ...
    r = client.put('/admin/config', json={'delivery_options': ['今天 20:00–22:00', '明天 08:00–10:00'], 'shipping_fee': 8}, headers=_h(admin_t))
    assert r.status_code == 200, r.text
...
def test_content_faqs_flow(client):
    admin_t = _admin_token(client)
    r = client.put('/admin/content/faqs', json={'faqs': [{'q': '测试问题', 'a': '测试答案'}]}, headers=_h(admin_t))
    assert r.status_code == 200
    r = client.get('/config')
    assert any(f['q'] == '测试问题' for f in r.json()['faqs'])
    r = client.put('/admin/content/announcements', json={'announcements': [{'content': '平台公告：春节正常营业'}]}, headers=_h(admin_t))
    assert r.status_code == 200
    r = client.get('/config')
...
def test_admin_shops_crud_and_location(client):
    ...
    admin_t = _admin_token(client)
    r = client.post('/admin/shops', json={'name': '后台测试花店', 'rating': 4.2, 'lat': 22.6, 'lng': 114.3, 'status': '营业中'}, headers=_h(admin_t))
    assert r.status_code == 200, r.text
    shop = r.json()['shop']
    sid = shop['shop_id']
    assert shop['name'] == '后台测试花店'
    assert shop['lat'] == 22.6
    r = client.put(f'/admin/shops/{sid}', json={'lat': 22.45, 'lng': 114.1, 'rating': 4.9, 'address': '深圳市福田区测试路 1 号'}, headers=_h(admin_t))
    assert r.status_code == 200, r.text
    upd = r.json()['shop']
    assert upd['lat'] == 22.45
    assert upd['lng'] == 114.1
