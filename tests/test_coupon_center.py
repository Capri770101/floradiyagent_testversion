...
def _register(client, username):
    r = client.post('/auth/register', json={'username': username, 'password': 'secret123', 'nickname': '领券测试'})
...
def test_redeem_with_points(client):
    ...
    assert r.status_code == 400
    r = client.post('/orders', headers=_h(token), json={'items': [{'plan_id': 'P001', 'name': 'x', 'price': 100, 'qty': 1, 'shop': 'S001'}]})
