...
def run_design_demo() -> None:
    ...
    cases = [('治愈系·送妈妈', '帮我做一个治愈系的花束送妈妈，预算200左右，希望她每天看到心情好'), ('失恋朋友·重生感', '朋友刚失恋，想送一束有『破晓重生』感觉的花，不要太俗气的红玫瑰'), ('妈妈生日·不俗气', '妈妈生日，想要惊喜感但不想太俗气，预算300，最好有她的名字元素')]
    plans = []
    ...
    print(f"\n{'=' * 72}\n  🔁 演示「改设计」追溯能力\n{'=' * 72}")
    plan0 = plans[0]
    new_plan_str = asyncio.run(revise_diy_plan(json.dumps(plan0, ensure_ascii=False), '主花换成向日葵，整体配色再明亮一点', _context={'user_id': 'demo_user'}))
...
async def run_chat_demo() -> None:
    ...
    agent = ReActAgent()
    turns = ['帮我做一个治愈系花束送妈妈，预算200', '主花换成向日葵，整体配色再明亮一点', '好，帮我生成效果图']
