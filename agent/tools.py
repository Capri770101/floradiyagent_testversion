...
async def search_plans(keyword: str, _context: dict | None=None) -> str:
    ...
    if not _req_clear(req):
        return json.dumps({'error': '需求还不明确，请先告诉我送给谁、什么场合或大概预算，我再为你挑选合适的花束。'}, ensure_ascii=False)
    location = None
    if _context:
...
def respond_to_user(reply: str='', ui: str='text', data: dict | None=None, stage: str='analyze', intent: str='other') -> str:
    ...
    if intent not in ('buying', 'qa', 'chitchat', 'design', 'other'):
        intent = 'other'
    return json.dumps({'reply': reply, 'ui': ui, 'data': data or {}, 'stage': stage, 'intent': intent}, ensure_ascii=False)
...
async def search_shops(plan: str='latest', _context: dict | None=None) -> str:
    ...
    req = _requirement_from_context(_context)
    if not _req_clear(req):
        return json.dumps({'error': '需求还不明确，请先告诉我送给谁、什么场合或大概预算，我再为你推荐合适的店铺。'}, ensure_ascii=False)
    location = None
    if _context:
        location = _context.get('location') or (req.location if req else None)
