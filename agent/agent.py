...
from agent import (
    skills,  # noqa: F401  —— 触发技能自动注册（create_order 等），仅副作用，名字不直接引用
)
...
class ReActAgent:
    ...
    def _build_system(self, stage: SessionStage, long_term: dict[str, str]) -> str:
        ...
        parts = [
            ...
            "- generate_effect_image：为已设计方案自动生成效果图（系统在方案设计完成后自动调用，无需询问用户）。",
            ...
            "   d) 用户选全新设计 / 无匹配模板 → 调 generate_diy_plan 设计专属方案（系统自动生成效果图随卡片下发）。",
            "   e) 设计完成后：先把方案卡展示给用户（含效果图），问一句『方案满意吗？可以的话我帮你匹配店铺下单』，"
            "   **等用户确认方案后**再调 match_shop_items / search_shops；不要设计完就抢推店铺。",
            "4. search_plans 返回为空或结果与需求明显不符时：不要硬推无关方案，直接转 DIY 流程（步骤 3）。",
            "5. DIY 方案设计完成后：系统会自动为方案生成效果图并随方案卡片下发，无需再询问用户是否需要效果图。"
            "用户确认方案后，调 match_shop_items 匹配店铺单品，再调 search_shops 推荐店铺。",
            "6. 用户选定店铺后：调 create_order 下单并给支付跳转。下单成功后方案自动沉淀为全局模板，供后续用户复用。",
            "## 原则",
            "1. 用户随时可打断、改需求、回退；不要强推固定流程。",
            "2. 方案设计完成后效果图会自动生成并随卡片下发；不要询问用户『是否需要生成效果图』。"
            "设计完先把方案卡展示给用户、等用户确认，确认后才推荐店铺/下单，不要设计完直接跳店铺。",
            "3. 下单前必须已推荐店铺且用户已选定。",
        ...
        parts.append(
            ...
            "- 设计完方案后：把方案卡展示给用户，简短说『方案已设计好，点击卡片可查看详情，效果图已自动生成』，"
            "然后问『方案满意吗？满意的话我帮你匹配店铺下单』，**等用户确认后再推进到店铺/下单**，不要抢推店铺；"
            "花材寓意等交给卡片。\n"
