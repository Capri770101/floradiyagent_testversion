...
def test_design_stores_plan_in_session() -> None:
    ...
    init_db()
    sid = asyncio.run(mem.get_or_create_session('u_diy_store'))
    ctx = {'user_id': 'u_diy_store', 'session_id': sid, 'location': None}
    data = json.loads(asyncio.run(tools.generate_diy_plan('探病祝福 清淡', ctx)))
    assert 'effect_prompt' in data
    stored = asyncio.run(mem.get_session_json('u_diy_store', sid, 'latest_diy_plan'))
    assert stored is not None
    assert stored['plan_id'] == data['plan_id']
    selected = asyncio.run(mem.get_session_json('u_diy_store', sid, 'selected_plan'))
    assert selected is not None and selected['plan_id'] == data['plan_id']
    assert json.loads(asyncio.run(tools.generate_diy_plan('自己悦己 100')))['diy'] is True

def test_generate_diy_plan_tool_returns_json() -> None:
    out = asyncio.run(tools.generate_diy_plan('自己悦己 北欧自然风 100'))
    data = json.loads(out)
...
def test_merge_keeps_llm_new_fields() -> None:
    ...
    llm = {'design': {'main_flowers': baseline['design']['main_flowers'], 'fillers': baseline['design']['fillers'], 'foliage': baseline['design']['foliage'], 'color_scheme': ['奶油白'], 'packaging': '礼盒', 'meaning': '母爱', 'difficulty': '高手', 'est_time': 60, 'shelf_life': '约 3-5 天', 'suitable_for': ['母亲'], 'caution': '花粉过敏者请谨慎接触', 'mood_tags': ['治愈', '感恩']}}
