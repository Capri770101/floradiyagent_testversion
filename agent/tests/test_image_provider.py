...
def test_download_image_to_local_magic_wins_over_content_type(tmp_path: pytest.TempPathFactory, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_download_image_to_local_rejects_non_image(tmp_path: pytest.TempPathFactory, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_create_image_task_api2img_sync_done(tmp_path: pytest.TempPathFactory, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fake_httpx.post.return_value = fake_resp
    monkeypatch.setattr(tasks, 'httpx', fake_httpx)
    tid = asyncio.run(tasks.create_image_task('a bouquet for mother'))
    row = asyncio.run(tasks.get_image_task(tid))
