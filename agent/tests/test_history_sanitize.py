...
def _fresh_user(uid: str) -> str:
    init_db()
    return mem.get_or_create_session(uid)
...
def test_mock_tool_calls_normalized_to_openai_schema() -> None:
    ...
    mem.save_messages(
        ...
        [
            ...
            {
                "role": "assistant",
                "tool_calls": [{"id": "", "name": "search_plans", "arguments": {"keyword": "玫瑰"}}],
            },
    ...
    assert isinstance(call["function"]["arguments"], str)  # arguments 必须是 JSON 字符串
...
def test_dirty_history_clean_pair_survives() -> None:
    ...
    mem.save_messages(
        sid,
        [
            {"role": "user", "content": "帮我设计"},
            {"role": "assistant", "tool_calls": [{"id": "", "name": "generate_diy_plan", "arguments": {}}]},
            {"role": "tool", "content": "{\"plan_id\": \"DIY_1\"}", "tool_call_id": ""},
