...
def test_plan_card_requires_plans_list():
    ...
    assert _V(UIType.PLAN_CARD, {}) is None
    assert _V(UIType.PLAN_CARD, {'plans': []}) is None
    assert _V(UIType.PLAN_CARD, {'plans': 'P001'}) is None
...
def test_image_task_requires_task_id_or_result():
    ...
    assert _V(UIType.IMAGE_TASK, {'task_id': None}) is None
