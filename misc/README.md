...
- **三端独立架构**：C 端 / 商家端 / 管理后台三个独立前端入口（同仓多 entry，构建出 `index.html` / `merchant.html` / `admin.html`），共享同一后端。三端令牌互相隔离（`floradiy_token` / `floradiy_merchant_token` / `floradiy_admin_token`）；商家走 `/auth/merchant-login`、管理走 `/auth/admin-login`，手机号全局唯一（`users.phone` 部分唯一索引兜底）。
...
```
flora_diy_agent/
├─ backend/               # 后端服务（uvicorn backend.api:app）
│  ├─ api.py              #   FastAPI 装配层（CORS/中间件/异常处理/lifespan + 挂载路由）
│  ├─ config.py           #   全部配置（含限流参数；读 misc/.env）
│  ├─ security.py         #   微信 code2session / JWT / 手机号验证码 / 账号密码 / 商家注册
│  ├─ review.py           #   内容机审（阶段5：开关预留 + 真实 API 挂载点，dev 放行）
│  ├─ routers/            #   按领域拆分（auth/chat/catalog/commerce/merchant/admin/notify/recommend/report）
...
├─ agent/                 # 智能体（agent.py 主类 + tools.py 工具注册表 + cli.py + requirements.py）
│  ├─ agent.py            #   智能体主类：ReAct 主循环 + skill 编排 + DIY 入库钩子
│  ├─ tools.py            #   工具注册表 + 内建工具
│  ├─ cli.py              #   本地调试 CLI（python agent/cli.py ...）
│  ├─ data/               #   智能体专属数据（agent_memory.db 长期记忆库）
│  ├─ tests/              #   智能体测试（13 文件：DIY 设计/迭代/资产库、知识库 RAG、生图、记忆净化…）
│  ├─ engine/             #   LLM 封装 / 状态机 / UI 协议
...
├─ H5/                    # React + Vite + Tailwind 移动端（跳舞兰视觉），三端三入口：
│  ├─ index.html+src/     #   C 端（顾客端，已剥离商家代码）
│  ├─ merchant.html+src/merchant/  #   商家端（独立令牌 floradiy_merchant_token）
│  ├─ admin.html+src/admin/        #   管理后台（独立令牌 floradiy_admin_token）
├─ tests/                 # 后端 API/业务测试（25 文件：鉴权/权限/订单/支付/限流/价格防篡改/管理后台/商家认证/内容举报）
├─ data/                  # 运行时落盘（agent_service.db 共享主库 / 生图 / 上传）
├─ misc/                  # 杂项与工程文件（本 README / .env / Dockerfile / Makefile / pyproject /
│                         #   .workbuddy / .github / docs / 任务书与设计文档）
└─ .gitignore             # 仓库忽略规则（保留在根目录）
```
...
```bash
cp misc/.env.example misc/.env   # 填 LLM_API_KEY 等
...
**验收标准**：
1. `python -m pytest -q -c misc/pyproject.toml` 全绿（当前 **263 passed**，含限流/价格防篡改/DIY 资产库/鉴权/权限/订单/支付/RAG/管理后台/商家认证/内容举报）。
